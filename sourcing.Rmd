...
tibble(resid = demands, distribution = demands2) %>% 
  ...
  geom_freqpoly()+
  labs(title = "Comparing two ways of generating demand")
```
...
```{r}
expected_profit <- function(Q, Ds = demands){
  price <- 135
  cost <- 65
...
diff_Qs <- tibble(
      ...
      e_profit = map_dbl(Qs, expected_profit))

ggplotly(       
       diff_Qs %>% 
  ggplot(aes(Qs, e_profit))+
  geom_line()+
  geom_vline(xintercept = 314, lty=2)+
  scale_y_continuous(labels = scales::dollar)+
  geom_text(data = data.frame(x = 355, y = 17300, label = "Source 315 from Ukraine proves to be most profitable"),mapping = aes(x = x, y = y, label = label), size = 3.86605783866058, alpha = 1, inherit.aes = FALSE)+
...
```{r}
expected_profit_local <- function(Q, Ds = demands){
  price <- 135
  cost_in <- 95
